  ...
  class CardProcessPresenter
    ...
    def checklist_string
      if checklist_activities.present?
        'In order to execute this activity, ' + task_subjects_string + ' must ' +  concatenate_commans_finish_with_and(checklist_activities.map(&:task)) + ' in parallel.'
      else
        ''
      end
    end
